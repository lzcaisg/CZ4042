...
for i in range(len(hidden_num_list)):
    ...
    with graph_list[i].as_default():
        ...
        train_graphName = "PartA-Train"+str(EPOCHS)+'-'+str(NUM_HIDDEN)+".png"
        test_graphName = "PartA-Test"+str(EPOCHS)+'-'+str(NUM_HIDDEN)+".png"
        plot_graph(EPOCHS, NUM_HIDDEN, train_acc_record,train_graphName, isTrain = True, error=True)
        plot_graph(test_count, NUM_HIDDEN, test_acc_record, test_graphName, isTrain = False)
        ...
        fileNameTail = str(EPOCHS)+'-'+str(NUM_HIDDEN)+".out"
